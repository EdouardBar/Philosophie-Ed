...
document.addEventListener('DOMContentLoaded', () => {
  ...
  emailjs.init("GqCSMwWHSlx-rDeW_");
  ...
  // Hier sammeln wir zusätzlich die vom Nutzer ausgewählten Antworttexte
  ...
  /**
   * Sendet per EmailJS alle gesammelten Antworten als E-Mail.
   */
  function sendAnswersByEmail() {
    const formattedAnswers = userSelectedAnswers
      .map((answer, index) => `Frage ${index + 1}: ${answer}`)
      .join("\n");
    ...
    emailjs
      .send("service_0h9oryn", "template_muo5fpp", templateParams, "GqCSMwWHSlx-rDeW_")
      .then(
        function (response) {
   ...
   * der Score aktualisiert und entweder die nächste Frage geladen oder, falls alle Fragen beantwortet wurden, das Ergebnis angezeigt.
   */
  nextButton.addEventListener('click', () => {
    ...
    if (selectedOption) {
      ...
      scores[category]++;
      // Speichere den sichtbaren Antworttext (Elternknoten des Inputs enthält auch den Text)
      userSelectedAnswers.push(selectedOption.parentNode.textContent.trim());
    }
    ...
    if (currentQuestionIndex < quizQuestions.length) {
      ...
      showResult();
      sendAnswersByEmail(); // Hier werden die Antworten per E-Mail verschickt
    }
  });
